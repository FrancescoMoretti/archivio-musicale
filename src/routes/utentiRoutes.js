...
router.get("/api/utenti", autenticaToken, autorizzaRuoli('superadmin', 'admin'), async (req, res)=>{
    let query=`SELECT u.id, u.email, u.nome, u.ruolo, u.created_at, c.email AS email_creatore, u.tentativi_falliti, COALESCE(u.bloccato_fino, '/') AS bloccato_fino, u.blocchi_consecutivi FROM utenti u LEFT JOIN utenti c ON u.created_by=c.id`;
    try{
